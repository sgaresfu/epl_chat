...
async def sign_in(body: LoginIn, request: Request, response: Response, state: State) -> MeOut:
    ...
    await check_rate_limit(state.cache, client_ip(request))

    person = verify_code(body.code, state.settings)
    if person is None:
        log.info("auth.rejected", ip=client_ip(request))
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="That code word is not one of the four.")
