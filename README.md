...
|---|---|---|
| `DATABASE_URL` | [neon.tech](https://neon.tech) → new project → *Connection string* (see below) | everything persistent |
...
Total cost: **nothing per month.** The static site is free, the api runs on
Render's free plan, and the cron jobs are billed per minute of runtime — a job
that finishes in seconds costs a fraction of a cent.

1. **Get a database.** Render's own free Postgres is deleted 30 days after
   creation, which lands mid-season, so use an external one.
   [Neon](https://neon.tech) → sign up → *Create project* → copy the connection
   string. It looks like
   `postgresql://user:pass@ep-xxx.eu-central-1.aws.neon.tech/neondb?sslmode=require`.
   Supabase works equally well.
2. **Create the blueprint.** Render → **Blueprints** → *New Blueprint Instance*
   → connect the repo. It reads `render.yaml` and creates the static site, the
   api and two cron jobs.
3. **Fill in five values** in the `league-secrets` group: `DATABASE_URL` from
   step 1, and the four code words. `SESSION_SECRET` is generated by Render.
   Everything else is optional — each missing key degrades one panel with a
   clear message.
4. **Set `FRONTEND_ORIGIN`** on the api service to the static site's URL, e.g.
   `https://league-web.onrender.com`. This one cannot be wired automatically:
   Render's `fromService … property: host` does not populate for a static site,
   and an empty allow-list makes the api reject every browser request while
   still answering health checks. The api refuses to start without it.

`VITE_API_BASE` is wired to the api by the blueprint, and migrations run in the
container entrypoint before the api takes traffic.

> If Render appends a suffix to the service names (`league-web-a1b2`), use the
> suffixed hostname. That happens when older services with the same names still
> exist.

### What the free plan costs you

The api **spins down after 15 minutes idle** and takes 30–60 seconds to wake.
While it sleeps the poller sleeps with it, so live scores refresh only once
somebody opens the page — fine for four people checking in, less so if you want
the table live all afternoon with nobody watching. Changing `plan: free` to
`plan: starter` on `league-api` (~$7/month) removes that entirely.

The cron jobs run in their own containers on their own schedule, so the Monday
snapshot happens whether or not the api is awake.

### Why this is not the architecture in the brief

BRIEF section 2 splits the poller into its own worker and puts Redis between
them, and gives good reasons: a slow upstream must not sit on a user's critical
path, and many browsers must not multiply upstream calls.

Neither reason binds at four users on one instance. asyncio already keeps a slow
FPL response off every other request, and one poll serves all four people
wherever it runs. What the split does cost is a $7 worker plus $10 of Redis
whose only job is letting two processes share a cache — $17/month of process
separation buying nothing observable.

So the poller runs inside the api (`POLLER_IN_PROCESS=true`) and the cache is
in-process. To restore the briefed architecture, set `POLLER_IN_PROCESS=false`,
add a `worker` service running `python -m services.poller.main`, and add a
`keyvalue` service supplying `REDIS_URL`. **No application code changes** — the
cache layer already picks Redis over memory whenever `REDIS_URL` reaches it.
`tests/test_deployment_shape.py` asserts the blueprint and the code agree.
